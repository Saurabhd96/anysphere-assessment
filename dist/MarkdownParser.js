...
function addToken(token) {
    if (!currentContainer || !currentTextNode)
        return;
    let text = token;
    ...
    for (let i = 0; i < text.length; i++) {
        ...
        if (codeBlock) {
            ...
            if (inlineCodeBlock) {
                ...
                currentTextNode.textContent += "`";
                currentTextNode = document.createTextNode("");
                currentContainer.appendChild(document.createTextNode("<code style='background-color: lightgray;'>"));
                currentContainer.appendChild(currentTextNode);
            ...
            i += headingLevel - 1;
            currentTextNode = document.createTextNode("");
            currentContainer.appendChild(document.createTextNode(`<h${headingLevel}>`));
            currentContainer.appendChild(currentTextNode);
        }
        else if (char === "-") {
            list = true;
